...
public class TaskServiceImpl implements TaskService {
    ...
    @Override
    public TaskResponse saveTasks(TaskRequest request) {
        ...
        if (lessons.getTest() == null) {
            Task task = taskMapper.convert(request);
    ...
    public TaskResponse delete(Long id) {
        Task task = taskRepository.findById(id).orElseThrow(() ->
                new NotFoundException(String.format("Task is not found id=%s", id)));
        Task resource_not_found_with_id = taskRepository.findById(id).orElseThrow(() ->
                new NotFoundException(String.format("resource not found =%s with id", id)));
    ...
    public TaskResponse getById(Long id) {
        Task task = taskRepository.findById(id).orElseThrow(() ->
                new NotFoundException("this id not found"));
        return taskMapper.deConvert(task);
    }
